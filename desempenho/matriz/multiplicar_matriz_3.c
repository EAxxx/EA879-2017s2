...
void transpor(matriz **A, matriz **B) {
  nova_matriz(B, (*A)->J, (*A)->I);
  float x;
  for (int i = 0; i < (*A)->I; i++) {
    for (int j = 0; j < (*A)->J; j++) {
      x = get_elemento(A, i, j);
      set_elemento(B, j, i, x);
    }
  }
...
void multiplicar(matriz **A, matriz **B, matriz **C) {
  ...
  for (i=0; i<(*C)->I; i++) {
    for (j=0; j<(*C)->J; j++) {
      x = 0;
      for (k=0; k<(*A)->J; k++) {
        x = x + get_elemento(A, i, k) * get_elemento(&Btransp, j, k);
      }
      set_elemento(C, i, j, x);
    }
  }

  deletar_matriz(&Btransp);
}
